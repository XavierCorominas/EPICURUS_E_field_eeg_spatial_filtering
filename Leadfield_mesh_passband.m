...
% The  script is organized in 3 blocks:
   ...
   % - 4* apply spatial filter to a eeg dataset 


 % Modify along the script the paths <>

 % Before starting, pelase install 
 % Simnibs4.0
 % ISO2mesh
 % EEGlab
 % Fieldtrip
...
addpath('<your_path>\SimNIBS-4.0\simnibs_env\Lib\site-packages\simnibs\matlab_tools\')

% Add path to ISO2Mesh
addpath('<your_path>\iso2mesh-1.9.6\')

% Add path to EEGLAB
eeglab_path = '<your_path>\eeglab2022.0';
addpath(eeglab_path);
eeglab;

% Add path to helsinki bem framework
addpath('<your_path>\External_functions\hbf_lc_p-master');
hbf_SetPaths


%% Define parameters

% Add MRI data info of the subject: example

subject_id = 'XX';
path_to_mri_nifti = ['<path_to_my_MRI_folder>,'\MRI'];
...
%T1 mri
filename = ['<T1mri_filename>.nii'];

%T2,FLAIR on IV1 image. Using a second contrst image generates better segmentations
filename_2 = ['<mri_filename>_INV1.nii'];
...
[status,cmdout] = system(['<path_to_simnibs>\SimNIBS-4.0\bin\charm',' ',subject_id,' ',filename,' ',filename_2,  ' --forceqform --forcerun'], '-echo');
...
cd(['<path_to_mesh>\m2m_XX\tdcs_leadfield\'])
...
filename = '<path_to_simnibs>\SimNIBS-4.0\simnibs_env\Lib\site-packages\simnibs\resources\ElectrodeCaps_MNI\EEG10-10_UI_Jurak_2007.csv';
delimiter = ',';
...
% Initialize variables.
filename = '<path_to_simnibs>\SimNIBS-4.0\simnibs_env\Lib\site-packages\simnibs\resources\ElectrodeCaps_MNI\Fiducials.csv';
delimiter = ',';
% Format for each line of text:
formatSpec = '%s%f%f%f%s%[^\n\r]';
% Open the text file.
...
%% Load a eeg dataset of your subjects to get electrode names and locations
...
%% The channel locations are defined in MNI coordinates. Transform to subject_coordinates

% point to m2m_ folder mesh
EEG_locs_subj =  mni2subject_coords(EEG_locs,([path_to_mri_nifti ,'\m2m_',subject_id,'\']));

fiducials =  mni2subject_coords(fiducials,([path_to_mri_nifti ,'\m2m_',subject_id,'\']));
...
%% Randomly visualize a source projection
close all;
...
%% Visualize source projections of eeg loaded data using a classical MNE estimator
...
%% Save the lead-fields for cortically constrained and free dipoles in the original reference:

save_folder = ['<path_to_save_fodler> \LEADFIELDS\'];
...
%% *************** 2nd part of the script: correct leadfields and cortex surface***********
...
% Correction tools
leadfield_corrections = ('<>\External_functions\leadfield_corrections')
...
path_to_leadfields = (['<>',subject_id,'\LEADFIELDS']);
...
%WORK WITH CORTICALLY CONSTRAIEND DIPOLES IN THIS PIPEPLINE.
...
%% Strip brainstem:
% if during the mesh generation you specifcy the --noneck atribute in
% charm, you might not need it.
...
%% Evalueate the head models again: 1. Plot the overall sensitivity to all the sources. Does any of the sources stand out?
...
%% Plot again to see the sutomatic corrrection results and Correct individual bad sources in the cortex if necessary:
...
%% *************** 3nd part of the script: construct the  passband ***********
...
addpath('<>\Analysis\External_functions')
...
mesh = mesh_load_gmsh4(['<>\m2m_',subject_id,'\',subject_id,'.msh']);
...
%% TMS Simulation for spatial filter. We here import a TMS position from the neuronavitaor and do a simulation on a previously marked TMS coil during the experimental session.
% You can also pecify the location manually withour importing. Pleasee see
% simnibs documentation for that.

% Import brainsight data and transform x axys (*be aware that X axis are inverted between
% brainsight and Simnibs space by default*). Check your neuronavigator
% coordinates.
importdata (['<>\NEURONAVIGATOR\Exported_Targets_s1.txt'])

%Transform brainsight to simnibs space
TMS_target = ans;
...
s.poslist{1}.pos(1).distance = 0; % if positions are imported this functions does not do anything. 
...
mesh_load_gmsh4(['<>,\MRI\simulation\',subject_id,'_corrected_TMS_1-0001_Magstim_70mm_Fig8_scalar.msh']);

% Create the binary mask (t) to create the spatial filter containing targeted structures
head = ans;

% As we created the leadfields based on the WM, here wee load 1001 which is
% WM E-field layer
cortex.e =  head.triangles(head.triangle_regions == 1001,:);
cortex.p =  head.nodes;

% Make shure head.elements_data {2,1} corresponds to norm_E.
...
%% Simulation full mesh: if you want to have a E-field mesh with all layers and not only WM
...
% Load simulation 
intensity = '90';
head = mesh_load_gmsh4(['<>'\MRI\simulationFull',intensity,'\',subject_id,'_TMS_1-0001_Magstim_70mm_Fig8_scalar.msh']);
