function [EEG_out] = EEGLAB_remove_and_interpolate_recharging_artifact(EEG_in, stim)
...
%[~, onset] = min(abs(EEG_in.times + 200));
...
plot(evoked_response', 'k');

xlim([onset end_of_interval]);
    
title('Zoom first in to show the whole artifact. Press enter when ready.')

pause();

title('Click the onset and ending of the artifact.')

user_input = ginput(2);

close(tmpfig);

x1 = floor(user_input(1,1));
x2 = ceil(user_input(2,1));

y1 = (setdiff(1:size(EEG_in.data,2), x1:x2 ));

if stim == 1    
    for i = 1:size(EEG_in.data,1)
    disp(['Interpolating the recharge artifact... Channel ',num2str(i)])
